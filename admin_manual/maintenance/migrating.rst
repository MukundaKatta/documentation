...
#.  Copy all files from your Nextcloud instance, the Nextcloud program files, the
    ...
    .. important::
      ...
      * ``datadirectory`` — **do not change this path.** Moving the data directory
        is not supported: it requires updating ``oc_storages`` entries in the database
        and will likely cause data corruption. If you truly have no choice, see
        :ref:`troubleshooting_data_directory` first.
